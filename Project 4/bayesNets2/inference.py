...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        myFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for join_VAR in eliminationOrder:
            myFactors, joinedFactor = joinFactorsByVariable(myFactors, join_VAR)
            if len(joinedFactor.unconditionedVariables()) > 1.0:
                elim_Factor = eliminate(joinedFactor, join_VAR)
                myFactors.append(elim_Factor)
        joinedFactor = joinFactors(myFactors)
