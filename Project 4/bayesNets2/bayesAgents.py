...
def constructBayesNet(gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            current = OBS_VAR_TEMPLATE % obsPos
            obsVars.append(current)
            edges.append((FOOD_HOUSE_VAR , current))
            edges.append((GHOST_HOUSE_VAR , current))
            variableDomainsDict[current] = OBS_VALS
...
def fillObsCPT(bayesNet, gameState):
    ...
    def getHouseVal(x,y):
        h = gameState.data.layout.height / 2.0
        w = gameState.data.layout.width / 2.0
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            current = OBS_VAR_TEMPLATE % obsPos
            obsVars.append(current)
            temp_factor = bn.Factor([current], HOUSE_VARS , bayesNet.variableDomainsDict())
            for assgn in temp_factor.getAllPossibleAssignmentDicts():
                R = 0 # initialize variables
                B = 0
                N = 0
                if assgn[FOOD_HOUSE_VAR] == getHouseVal(obsPos[0] , obsPos[1]):
                    R = PROB_FOOD_RED
                    B = 1 - PROB_FOOD_RED
                    N = 0
                elif assgn[GHOST_HOUSE_VAR] == getHouseVal(obsPos[0] , obsPos[1]) :
                    R = PROB_GHOST_RED
                    B = 1 - PROB_GHOST_RED
                    N = 0
                else:
                ...
                if assgn[current] == RED_OBS_VAL :
                    temp_factor.setProbability(assgn, R )
                elif assgn[current] ==  BLUE_OBS_VAL :
                    temp_factor.setProbability(assgn, B )
                else:
                    temp_factor.setProbability(assgn , N )

                        
   
            bayesNet.setCPT(current , temp_factor)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    max = float('-inf')
    max_assgn = dict()
    for assgn in factor.getAllPossibleAssignmentDicts():
        prob = factor.getProbability(assgn)
        if prob > max:
            max = prob
            max_assgn = assgn
    return max_assgn
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        tl.update(evidence)
        tl[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        tl[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL
        tr.update(evidence)
        tr[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        tr[GHOST_HOUSE_VAR] = TOP_LEFT_VAL
        
        
        leftExpectedValue = CPT.getProbability(tl) * WON_GAME_REWARD + CPT.getProbability(tr) * GHOST_COLLISION_REWARD
        
        
        
        rightExpectedValue = CPT.getProbability(tl) * GHOST_COLLISION_REWARD + CPT.getProbability(tr) * WON_GAME_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        pao = self.getExplorationProbsAndOutcomes(evidence)
        
        for pb in pao:
            p,e = pb
            expectedValue = expectedValue + p * max(self.computeEnterValues(e, enterEliminationOrder))
